...
_NOTE_: Fragments that are closer to the viewer will typically have lower depth values, while fragments that are farther
away will have higher depth values. Each fragment generated during rendering is associated with a depth value, typically
representing the distance from the viewer's eye to the fragment's position in the scene.
