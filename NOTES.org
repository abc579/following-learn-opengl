...
*** Ambient Lighting
It consists on adding some light to the final colour of object's fragments. This way, we simulate that there are light
particles scattered all around, even when there isn't any light source in the scene.

*** Diffuse Lighting
Consists on giving the object more brightness the closer the object's fragments are from the light source and also
taking into consideration the direction of light's rays, e.g, if a side of a cube is facing the light source and is very
close to it, it will be very bright. But the backside of that cube will be almost black (not completely, because ambient
lighting), because light's rays are not getting there.

When computing diffuse lighting we need to get the angle between the light source ray and the object's fragment. This is
because, if it is 90º, i.e, the light source is perpendicular to the object's fragment, the impact will be the
highest. The angle, then, is a parameter that we need to give more or less colour to the fragment.

To measure that angle we use a [[Normal Vector][normal vector]], which, in this case, is a vector that is perpendicular to the object's
fragment surface.

The angle, then, is computed by a dot product bewteen the two vectors. _NOTE_: both vectors have to be normalized first!
Because otherwise, the dot product will return something more than the cosine.

When the angle between the vectors tends to 0º, the more impact light will have over that fragment. Likewise, when the
angle tends to 90º, light will have less impact (almost nothing) over that fragment.

The two things we need:

- *Normal vector*: a vector that is perpendicular to the vertex' surface.
- *The directed light ray*: a direction vector that is the difference vector between the light's position and the
  fragment's position. To calculate this light ray we need the light's position vector and the fragment's position
  vector.

_NOTE_: computing local coordinates (vertex position attributes) with the view matrix gives us world space coordinates.

*** Normal Vector
Vector that is perpendicular to the fragment's surface.

TODO: how to compute it? because there's no such a thing as a vertex surface... it's about using the cross product with
nearby vertices....
...
reflection on objects and much more stuff. They're expensive to compute, though.
...
Lesson 4: Vector Addition and Subtraction
